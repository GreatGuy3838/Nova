...
module.exports = {
  ...
  run: async (client, message, args, cmduser, text, prefix) => {
    ...
    return message.channel.send(new MessageEmbed()
        ...
        .setDescription(`Usage: \`${prefix}removemoney <@USER> <Amount>\`\n\n\Example: \`${prefix}removemoney <@757243549774381099> 42069\``)
      );
    topay = topay.user;
    ...
    if(!payamount)
      return message.channel.send(new MessageEmbed()
        .setColor(es.wrongcolor)
        .setFooter(user.tag, user.displayAvatarURL({dynamic: true}))
        .setTitle(`<:RedNope:914312373890854922> You didn't remove the payamount`)
    ...
    return message.channel.send(new MessageEmbed()
        .setColor(es.wrongcolor)
        .setFooter(user.tag, user.displayAvatarURL({dynamic: true}))
        .setTitle(`<:RedNope:914312373890854922> You can't remove a negative Amount of Money or no Money, to ${topay}`)
      );
    ...
    if(payamount > data2.balance)
      return message.channel.send(new MessageEmbed()
        .setColor(es.wrongcolor)
        .setFooter(user.tag, user.displayAvatarURL({dynamic: true}))
        .setTitle(`<:RedNope:914312373890854922> You can't remove more Money than you he has in his/her **👛 Pocket (\`${data2.balance} 💸\`)**`)
      );
    ...
    return message.reply(new MessageEmbed()
      .setColor(es.color)
      .setFooter(user.tag, user.displayAvatarURL({dynamic: true}))
      .setTitle(`<:GreenCheck:914034845112479774> **You removeed \`${payamount} 💸\` to \`${topay.tag}\`**`)
